...
- Runtime controls are only what the caller gives **as controls** — a provider, a workspace, a run to resume, `--max-rounds`, `--session`, `--new`, `--on-sleep`, `--model`, `--thinking`, `--unverified-provider` (Kimi needs it) — stated before or apart from the task. Pass the workspace as `--workspace '<dir>'` and the agent options after a bare `--`. **Never remove flag-like words from the task itself**: a task about a CLI names its flags (`--provider deepseek`, `--session`, a lone `--`) and they must reach wchat verbatim.
...
- Reply by outcome: `created` — the script's stdout as-is plus the `NEXT:` line; `refused` — its stdout, then its stderr and exit code verbatim; `unknown` — its stdout as-is, including the not-known warning. No other commentary.
